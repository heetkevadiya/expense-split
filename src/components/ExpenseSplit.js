...
function ExpenseSplit({ group, onBack }) {
  const { groupName, members } = group;
  const [totalAmount, setTotalAmount] = useState('');
  const [description, setDescription] = useState('');
  const [paidBy, setPaidBy] = useState('');
  const [splits, setSplits] = useState(() =>members.map((m) => ({ name: m, selected: true })));
        ...
        {(() => {
          ...
          return splits.map((s, i) => (
            <div key={s.name} style={{ marginBottom: '8px' }}>
              ...
              {s.selected && (
                <span style={{ marginLeft: '10px' }}>₹{splitValue.toFixed(2)}</span>
              )}
            </div>
          ));
        })()}
      ...
      {expenses.length > 0 && (
        <div className="display-section" style={{ marginTop: '20px' }}>
          ...
          <ul>
            {expenses.map((exp, i) => (
              <li key={i}>
                {exp.description}: ₹{exp.amount.toFixed(2)} paid by {exp.paidBy}, split among{' '}
                {exp.splits.map((s) => `${s.name} (₹${s.value.toFixed(2)})`).join(', ')}
              </li>
            ))}
          </ul>
          <button onClick={calculateOverallSettlements}>Calculate Overall Settlements</button>
